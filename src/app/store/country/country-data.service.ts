...
export class CountryDataService extends DefaultDataService<Country> {
  ...
  getAll(): Observable<Country[]> {
    this.coronavirusApiService.getCurrentCountryTimelineCases({ country: 'Mexico', status: 'deaths' }).pipe(
      pluck('All', 'dates'),
      map(res => {
        return {
      ...
      mergeMap(case1 => this.coronavirusApiService.getCurrentCountryTimelineCases({ country: 'Mexico', status: 'confirmed' }).pipe(
        pluck('All', 'dates'),
        map(res => {
          return {
            deaths: case1.deaths,
      ...
      mergeMap(case2 => this.coronavirusApiService.getCurrentCountryTimelineCases({ country: 'Mexico', status: 'recovered' }).pipe(
        pluck('All', 'dates'),
        map(res => {
          return {
    ...
    return this.coronavirusApiService.getCurrentCases();
  }
