...
export class WorldmapComponent implements OnInit, AfterViewInit {
  @Input() worldTimeline: { date: string, list: { active: number, confirmed: number, deaths: number, recovered: number, id: string }[] }[];
  @Input() totalTimeline: { active: number, date: string, confirmed: number, deaths: number, recovered: number, id: string }[];
  constructor() { }
  ...
  ngAfterViewInit(): void {
    var covid_worldTimeline = this.worldTimeline;
    var covid_totalTimeline = this.totalTimeline;
    ...
    var activeColor = am4core.color("#1c5fe5");
    var confirmedColor = am4core.color("#cccc00");
    var recoveredColor = am4core.color("#35aa35");
    ...
    var countryColor = am4core.color("rgba(137,152,128,0.4)");
    var countryStrokeColor = am4core.color("rgb(0,0,0,0.2)");
    var buttonStrokeColor = am4core.color("rgba(37,52,28,1)");
    var countryHoverColor = am4core.color("rgba(87, 102, 78, 1)");

    var currentIndex;
    var currentCountry = "World";

    // last date of the data
    var lastDate = new Date(covid_worldTimeline[covid_worldTimeline.length - 1].date);
    ...
    var countryIndexMap = {};
    var list = covid_worldTimeline[covid_worldTimeline.length - 1].list;
    ...
    for (var i = 0; i < covid_totalTimeline.length; i++) {
      var di = covid_totalTimeline[i];
    ...
    var slideData = covid_worldTimeline[index ? index : covid_worldTimeline.length - 1];

    // augment with names
    for (var i = 0; i < slideData.list.length; i++) {
      slideData.list[i]['name'] = idToName(slideData.list[i].id);
    }
    ...
    for (var i = 0; i < mapData.length; i++) {
      var di: { date: string, active: number, confirmed: number, deaths: number, recovered: number, id: string } = mapData[i];
    ...
    container.tooltip.fontSize = "1em";
    ...
    mapChart.seriesContainer.background.fillOpacity = 0.0;
    ...
    mapChart.geodataNames = am4geodata_lang_ES;
    // Set projection
    ...
    polygonSeries.heatRules.push({
      ...
      property: "fill",
      min: am4core.color("rgb(220,220,220)"),
      max: colors[currentType],
      dataField: "value"
    })


    var activeCountryColor = colors[currentType];
    ...
    bubbleSeries.tooltip.showInViewport = true;
    ...
    /*  var title = mapChart.titles.create();
     ...
     title.text = "COVID-19 Distribución de casos reportados.";
     title.align = "left";
    ...
    mapGlobeSwitch.leftLabel.fill = am4core.color("#222");
    mapGlobeSwitch.rightLabel.fill = am4core.color("#222");
    mapGlobeSwitch.rightLabel.text = "Globo";
    ...
    absolutePerCapitaSwitch.y = 35;
    absolutePerCapitaSwitch.leftLabel.text = "Casos";
    absolutePerCapitaSwitch.leftLabel.fill = am4core.color("#000");
    absolutePerCapitaSwitch.rightLabel.fill = am4core.color("#000");
    absolutePerCapitaSwitch.rightLabel.text = "Casos por millón";
    absolutePerCapitaSwitch.rightLabel.interactionsEnabled = true;
    absolutePerCapitaSwitch.rightLabel.tooltipText = "Al calcular el valor máximo, no se incluyen los países con una población inferior a 100,000 habitantes."
    ...
    countryName.fill = am4core.color("#000");
    ...
    chartAndSliderContainer.background.fillOpacity = 0.0;
    chartAndSliderContainer.paddingTop = 20;
    ...
    slider.events.on("rangechanged", (event) => {
      var index = Math.round((covid_worldTimeline.length - 1) * slider.start);
      updateMapData(covid_worldTimeline[index ? index : covid_worldTimeline.length - 1].list);
      updateTotals(index);
    })
    ...
    slider.startGrip.icon.stroke = am4core.color("#000");
    ...
    sizeSlider.background.fill = am4core.color("#000");
    sizeSlider.adapter.add("y", (y, target) => {
    ...
    sizeSlider.startGrip.icon.stroke = am4core.color("#666");
    ...
    sizeLabel.text = "Ajuste de radio circular*";
    sizeLabel.fill = am4core.color("#000");
    sizeLabel.rotation = 0;
    sizeLabel.fontSize = "10px";
    sizeLabel.fillOpacity = 1;
    ...
    sizeLabel.tooltipText = "Algunos países tienen tantos casos que las burbujas para países con valores más pequeños a menudo tienen el mismo aspecto, incluso si hay una diferencia significativa entre ellos. Este control deslizante se puede usar para aumentar el tamaño máximo de una burbuja de modo que cuando se acerque a una región con valores relativamente pequeños, pueda compararlos de todos modos."
    //sizeLabel.fill = am4core.color("#666");
    ...
    filterSlider.height = am4core.percent(20);
    filterSlider.marginLeft = 25;
    filterSlider.align = "left";
    filterSlider.valign = "top";
    filterSlider.verticalCenter = "middle";
    filterSlider.opacity = 0.8;
    filterSlider.background.fill = am4core.color("rgb(200,200,200)");
    filterSlider.adapter.add("y", (y, target) => {
    ...
    filterSlider.startGrip.icon.stroke = am4core.color("#000");
    ...
    filterLabel.text = "Filtro de valores máximos *";
    filterLabel.rotation = 0;
    filterLabel.fontSize = "1em";
    filterLabel.fill = am4core.color("#000");
    filterLabel.fontSize = "0.8em";
    filterLabel.fillOpacity = 0.8;
    filterLabel.horizontalCenter = "middle";
    filterLabel.align = "left"
    filterLabel.paddingBottom = 20;
    filterLabel.tooltip.label.wrap = true;
    filterLabel.tooltip.label.maxWidth = 300;
    filterLabel.tooltipText = "Este filtro permite desactivar del mapa la visualización de países con muchos casos reportados para que sea posible comparar países con menor número de casos."
    //filterLabel.fill = am4core.color("#666");
    ...
    lineChart.data = JSON.parse(JSON.stringify(covid_totalTimeline));

    // date axis
    // https://www.amcharts.com/docs/v4/concepts/axes/date-axis/
    var dateAxis = lineChart.xAxes.push(new am4charts.DateAxis());
    dateAxis.renderer.minGridDistance = 50;
    dateAxis.renderer.grid.template.stroke = am4core.color("#666");
    ...
    valueAxis.renderer.grid.template.stroke = am4core.color("#666");
    ...
    lineChart.legend.labels.template.fill = am4core.color("#000");
    lineChart.legend.markers.template.height = 12;
    lineChart.legend.contentAlign = "left";
    lineChart.legend.fontSize = "1em";
    ...
    seriesTypeSwitch.leftLabel.text = "Casos acumulados";
    seriesTypeSwitch.rightLabel.text = "Casos diarios"
    seriesTypeSwitch.leftLabel.fill = am4core.color("#000");
    seriesTypeSwitch.rightLabel.fill = am4core.color("#000");
    ...
    function addSeries(name, color) {
      ...
      series.tooltip.background.fill = am4core.color("#555");
      series.tooltip.dy = -4;
    ...
    label.text = "Las estadisticas más recientes están sujetas a la entrega de las fuentes de datos.";
    ...
    function addButton(name, color) {
      ...
      button.label.fill = am4core.color("#666");
      //button.label.fontSize = "11px";
      button.background.cornerRadius(10, 10, 10, 10);
    ...
    function setCountryData(countryIndex) {
      ...
      for (var i = 0; i < lineChart.data.length; i++) {
        var di = covid_worldTimeline[i].list;
    ...
    // update total values in buttonsTimeLine
    function updateTotals(index?) {

      var di = covid_totalTimeline[index ? index : covid_totalTimeline.length - 1];;
      ...
      for (var key in buttons) {
        var count = Number(lineChart.data[index ? index : covid_totalTimeline.length - 1][key])
        if (!isNaN(count)) {
