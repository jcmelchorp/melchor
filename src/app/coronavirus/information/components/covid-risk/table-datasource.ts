...
export class TableDataSource extends DataSource<OccupationRisk> {
   ...
   */
  connect(): Observable<OccupationRisk[]> {
    // Combine everything that affects the rendered data into one update
    // stream for the data-table to consume.
    const dataMutations = [
      observableOf(this.data.risk),
      this.paginator.page,
    ...
    return merge(...dataMutations).pipe(
      map(() => {
        return this.getPagedData(this.getSortedData([...this.data.risk]));
      })
    );
  }
   ...
   */
  private getPagedData(data: OccupationRisk[]) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: OccupationRisk[]) {
